...
def find_possible(robots, inventory, reqs):
    ...
    if reqs["clay_req3"] <= robots["clay"]:
        new_clay_r = 2 * TIME_LIMIT

    if reqs["obs_req4"] <= robots["obs"]:
        new_obs_r = 2 * TIME_LIMIT

    if all(reqs[f'ore_req{i}'] <= robots['ore'] for i in range(1, 5)):
...
def dfs(time, robots, inventory, reqs, results):
    time_skips = find_possible(robots, inventory, reqs)
